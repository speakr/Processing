 public class TerrainGen {
    ...
    public float[][] generate() {
        ...
        //make corners negative for islands
        map[0][0] = random(1);
    ...
    public float[][] generate(float[][] map) {
        ...
        private float averageDiamond(float[][] map, int x, int y, int step) {
            ...
            return (average/count)%average;
        }
    private float randVariance(float v) {
        return (random(1)*2*v - v)%pow(random(v),-2); //this can end up being negative, resulting in huge sections of negative numbers. Handled in setupTerrain.
    }
}
